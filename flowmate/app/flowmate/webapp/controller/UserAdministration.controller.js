sap.ui.define([
    ...
    return BaseController.extend("flowmate.controller.UserAdministration", {
        ...
        async onOpenEditDialog(oEvent) {
            ...
            const oEditUser = { ...this._emptyUser(), ...oUser };
        ...
        async onSaveUser() {
            ...
            try {
                ...
                const oSavedUser = bIsEdit
                    ? (await this.updateEntry(`/Users(guid'${oUser.ID}')`, oPayload), { ...oUser, ...oPayload })
                    : await this.createEntry("/Users", oPayload);

                if (oUser.team_ID) {
                    await this._ensureTeamMembership(oUser.team_ID, oSavedUser.ID || oUser.ID);
                }
